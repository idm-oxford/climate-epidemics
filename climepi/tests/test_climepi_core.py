...
from climepi import ClimEpiDatasetAccessor
...
def test_annual_mean():
    ...
    temp_am_exp = np.array([
        np.sum(temp[:,0:12]*np.array([31,29,31,30,31,30,31,31,30,31,30,31]),axis=1)/366,
        np.sum(temp[:,12:24]*np.array([31,28,31,30,31,30,31,31,30,31,30,31]),axis=1)/365
        ]).T
...
def test_ensemble_mean():
    ...
    ds = xr.Dataset(
        ...
        coords={'realization': np.array([1,2]),'hello': np.array(['there','general','kenobi'])}
        )
...
def test_ensemble_percentiles():
    """Test the ensemble_percentiles method of the ClimEpiDatasetAccessor class."""
    temp = np.array([[1,2,3],[4,5,6]])
    ds = xr.Dataset(
        {'temp': (('realization', 'hello'), temp)},
        coords={'realization': np.array([1,2]),'hello': np.array(['there','general','kenobi'])}
    ...
    ds_percentiles_exp = xr.Dataset(
        ...
        coords={'hello': np.array(['there','general','kenobi']),'percentile':percentiles}
        )
    xrt.assert_allclose(ds_percentiles, ds_percentiles_exp)

def test_ensemble_mean_std_max_min():
    """Test the ensemble_mean_std_max_min method of the ClimEpiDatasetAccessor class."""
    temp = np.array([[1,2,3],[4,5,6]])
    ds = xr.Dataset(
        {'temp': (('realization', 'hello'), temp)},
        coords={'realization': np.array([1,2]),'hello': np.array(['there','general','kenobi'])}
    ...
    temp_stat_exp = np.array([temp_mean_exp,temp_std_exp,temp_max_exp,temp_min_exp]).T
...
def test_ensemble_stats():
    ...
    temp_stat_exp = np.array(
        [temp_mean_exp,temp_std_exp,temp_max_exp,temp_min_exp,temp_lower_exp,temp_median_exp,
         temp_upper_exp]
        ).T
    ds_stat_exp = xr.Dataset(
        {'temp': (('hello','ensemble_statistic'), temp_stat_exp)},
