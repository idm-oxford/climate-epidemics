...
and planning public health interventions. However, a recent review paper highlighted
...
can be used to access climate data from an external source, or load a pre-defined
example dataset. (2) The `epimod` subpackage can be used to define a mechanistic
suitability model, infer temperature response curves of trait parameters using
laboratory data (left plot), and determine the overall temperature dependence of the
suitability metric (right plot). Pre-defined example models are also available. (3) The
`climepi` accessor for xarray datasets can be used to combine a climate dataset with a
suitability model to obtain suitability projections, and to assess the contributions of
different sources of climate uncertainty and visualize results. See the package
[documentation](https://climate-epidemics.readthedocs.io/en/stable/gallery.html) for
detailed usage examples.](schematic.pdf)

2. Epidemiological model (`epimod`) subpackage: provides classes and methods for
   models of climate suitability for VBD transmission, in which a suitability metric
...
variability, to uncertainty in future climate suitability for dengue in a range of
locations that do not currently experience substantial transmission
...
suitability model developed by @mordecai_detecting_2017, and recreating the analysis of
the impact of internal climate variability on climate suitability for *Aedes aegypti*
(a vector of VBDs including dengue and yellow fever), by @kaye_impact_2024.
